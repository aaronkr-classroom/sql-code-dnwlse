...
3. 데이터베이스 조작어를 호스트 언어에 포함하여 데이터베이스에 접근 (정답)
...
2. 모든 사용자가 제한 없이 자유롭게 접근할 수 있도록 데이터를 관리한다. (정답)
...
3. 사용자가 시스템 카탈로그를 직접 갱신할 수 있다. (정답)
...
2. 내부 스키마는 개별 사용자가 보는 개인적인 데이터베이스 구조이다. (정답)
...
파일 시스템 (1950~1960년대): 각 응용 프로그램이 독립적으로 파일을 생성하고 관리
계층형 데이터베이스 (1960~1970년대): 데이터를 트리 구조로 조직. 부모-자식 관계로 데이터를 표현
네트워크 데이터베이스 (1970년대): 데이터가 그래프 구조로 표현되어 복잡한 관계를 관리
관계형 데이터베이스 (1970~1980년대): 데이터를 테이블 형태로 저장, SQL을 통해 데이터 접근
객체지향 데이터베이스 (1980~1990년대): 객체지향 프로그래밍 패러다임을 데이터베이스에 통합
NoSQL 데이터베이스 (2000년대 이후): 비정형 및 반정형 데이터를 처리할 수 있도록 다양한 데이터 모델 제공
클라우드 데이터베이스 및 분산 데이터베이스 (2010년대 이후): 클라우드 환경에서 데이터베이스 서비스를 제공하며, 분산 처리 가능

**19 데이터 사전(data dictionary)과 데이터 디렉토리 (data directory)의 차이점을 설명하시오.**
데이터 사전은 데이터의 메타데이터 및 정의를 포함하고, 데이터 디렉토리는 데이터의 물리적 위치를 나열하는 목록입니다.
